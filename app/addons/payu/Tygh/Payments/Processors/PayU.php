...
class PayU
{
    const LU_URL = 'https://secure.ypmn.ru/order/lu.php';
    const TOKEN_PAYMENT_URL = 'https://secure.ypmn.ru/order/tokens/';
    const IDN_URL = 'https://secure.ypmn.ru/order/idn.php';
    const IRN_URL = 'https://secure.ypmn.ru/order/irn.php';
    const PAYOUT_LINK_CARD_URL = 'https://secure.ypmn.ru/order/pwa/service.php/UTF/NewPayoutCard';
    const PAYOUT_URL = 'https://secure.ypmn.ru/order/prepaid/NewCardPayout';
    const IOS_URL = 'https://secure.ypmn.ru/order/ios.php';
    ...
    public static function writeLog($data, $file = 'payu_request.log')
    {
    ...
    public function calculateIPNHash()
    {
        $hash = '';
        $params = $_POST;
        unset($params['HASH']);
        foreach ($params as $item) {
            if (is_array($item)) {
                foreach ($item as $elem) {
                    $hash .= mb_strlen($elem, '8bit').$elem;
                }
            } else {
                $hash .= mb_strlen($item, '8bit').$item;
            }
        }
        return hash_hmac('md5', $hash, $this->secretKey);
    }
